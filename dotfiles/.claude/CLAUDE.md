...
Running `pnpm install` in a Jetpack checkout is fine, no need to ask. Two things worth knowing: in the primary checkout the host and the Docker container share one bind-mounted `node_modules`, so installing for one platform replaces the other's native binaries (darwin-arm64 vs linux-arm64) and breaks its builds until reinstalled. Conductor worktrees have their own `node_modules` and don't have this conflict — a fresh worktree usually needs an install before the `jetpack` CLI will run at all. `jetpack docker phpunit` is unaffected either way, because Composer packages are platform-independent.
